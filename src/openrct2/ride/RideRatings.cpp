...
namespace OpenRCT2
{
    ...
    static void RideRatingsApplyBonusSlideUnlimitedRides(
        RideRating::Tuple& ratings, const Ride& ride, RatingsModifier modifier);
    ...
    static void SetUnreliabilityFactor(Ride& ride)
    {
        ...
        if (rtd.flags.has(RtdFlag::reverseInclineLaunchAffectsReliability)
...
// Very high lateral G force penalty
    ...
    static int32_t ride_ratings_get_scenery_score(const Ride& ride)
    {
        ...
        {
            for (int32_t xx = std::max(tileLocation.x - 5, 0); xx <= std::min(tileLocation.x + 5, gameState.mapSize.x - 1);
                 xx++)
            {
...
#pragma region Ride rating calculation helpers
    ...
    static void RideRatingsApplyBonusLength(RideRating::Tuple& ratings, const Ride& ride, RatingsModifier modifier)
    {
        RideRatingsAdd(
            ratings,
            (std::min(ToHumanReadableRideLength(ride.getTotalLength()), modifier.threshold) * modifier.excitement) >> 16, 0, 0);
    }
    ...
    {
        RideRatingsAdd(
            ratings, ride.rotations * modifier.excitement, ride.rotations * modifier.intensity,
            ride.rotations * modifier.nausea);
    }
    ...
    static void RideRatingsApplyBonusHoles(RideRating::Tuple& ratings, const Ride& ride, RatingsModifier modifier)
    {
        ...
        RideRatingsAdd(
            ratings, bonusHoles * modifier.excitement, bonusHoles * modifier.intensity, bonusHoles * modifier.nausea);
    }
