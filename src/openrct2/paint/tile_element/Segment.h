...
namespace OpenRCT2
{
    ...
    constexpr PaintSegments kSegmentsAll(
    ...
    constexpr PaintSegments paintSegmentsRotate(const PaintSegments segments, const uint8_t rotation)
    {
        uint8_t outerSegments = segments.holder & 0xFF;
        outerSegments = Numerics::rol8(outerSegments, rotation * 2);
        return PaintSegments(
            static_cast<uint16_t>((segments.holder & PaintSegments(PaintSegment::centre).holder) | outerSegments));
    }

    constexpr PaintSegments paintSegmentsFlipXAxis(const PaintSegments segments)
    {
        uint8_t outerSegments = segments.holder & 0xFF;
